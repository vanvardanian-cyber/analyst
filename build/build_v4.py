#!/usr/bin/env python3
"""Build v4: full Amazon.de seller workbook — v3 + multi-year plan + Cerebro-analysis upgrades."""
import datetime
...
put(ws, "B2", "Amazon.de Seller Workbook — v4", F_TITLE)
...
nav = [
 ...
 ("12 Multi-year plan", "Do 3 years of staggered launches stay funded?",
  "IFERROR(\"lowest cash across 12 quarters \"&TEXT('12 Multi-year plan'!C32,\"#,##0\")&\" €\",\"fill Sheet 12\")"),
]
...
put(ws, "B23", "New product path:  1 → 2 → 3 → 4  (run 5 to compare several niches)  → 6 → 7 → 8, then order.", F_SECTION)
put(ws, "B24", "Already selling:  9 → 10 → 11 once a month, 7 every two weeks. Planning the business, not one product: 12.", F_SECTION)
note(ws, "B26", "v4 · 2026-08-25 · Built for a non-EU (deemed supplier) seller on Amazon.de. Educational tool, not tax or legal advice.")
...
guide_rows = [
 ...
 (25, "What changed in v4", F_SECTION),
 (26, "Added: Sheet 12 multi-year plan — 12 quarters, up to 3 staggered product launches with ramp-up, lowest-cash check (the integrated-model skeleton from the Freedom Ticket financial module, rebuilt on this file's German deemed-supplier unit economics) · review-wall-in-months gauge + textile/plant-material compliance reminder (Sheet 4) · P10/P90 demand helper from Cerebro keyword sales and position share (Sheet 6) · CPR launch-push cost estimator (Sheet 7) · cash cycle & cash turns per year (Sheet 8).", F_BASE),
 (28, "What changed in v3", F_SECTION),
 (29, "Added: Start dashboard with live status per sheet · Sheet 6 first-order size (newsvendor) · Sheet 7 PPC planner with breakeven/target ACOS, TACOS, ROAS and a search-term waste finder · Sheet 8 twelve-month cash-flow plan incl. import-VAT refund timing · Sheet 9 monthly P&L with TACOS tracking · Sheet 10 money-recovery audit (lost inventory matcher + FBA fee sanity check) · Sheet 11 review-theme analysis + fake-review checklist · (i) comments and back-links everywhere.", F_BASE),
 (31, "Carried over from v2 (fixes to the original file): duty on CIF base · breakeven ACOS vs VAT-inclusive price · max EXW supplier ceiling · Georgian 1% turnover tax · Dec→Jan ramp wrap · incomplete-data guard on the seasonality verdict · IP + PPWR rows in the selection gates.", F_BASE),
 (33, "This file is an educational tool, not tax, legal or investment advice. Amazon fees change every December/January — cross-check your category's rates in Seller Central before big decisions.", F_NOTE),
]
...
put(ws, "B38", "Review wall — how long until you catch up? (v4)", F_SECTION, fill=FILL_HDR); put(ws, "C38", "", F_SECTION, fill=FILL_HDR)
put(ws, "B39", "Median review count of page-1 top ASINs", F_BASE); inp(ws, "C39", 75, fmt="0")
note(ws, "E39", "Xray: sort page 1 by rank, take the median review count of the top 10 — not the average (one 5,000-review giant distorts it).")
put(ws, "B40", "Your expected units sold per month", F_BASE); inp(ws, "C40", 300, fmt="0")
put(ws, "B41", "Review rate (reviews per unit sold)", F_BASE); inp(ws, "C41", 0.015, fmt="0.0%")
note(ws, "E41", "Organic review rate on Amazon.de runs 1–2% of units; Vine (30 units) adds a one-off ~25 at the start.")
put(ws, "B42", "Reviews you gain per month", F_BASE); frm(ws, "C42", '=IFERROR(C40*C41,"")', fmt="0.0")
put(ws, "B43", "Months to reach the page-1 median", F_BOLD); frm(ws, "C43", '=IFERROR(C39/C42,"")', fmt="0.0", font=F_BOLD)
frm(ws, "B44", '=IF(C43="","",IF(C43<=6,"OK — the wall is reachable within ~6 months.",IF(C43<=12,"CAUTION — about a year of grinding at a review deficit; budget PPC above target ACOS for that long.","FLAG — over a year behind on social proof. You need Vine + a real differentiation angle (Sheet 11), or a lower-wall niche.")))', font=F_BOLD)
comment(ws, "C43", "A review count is a WALL, not just a score: buyers sort and filter by it, and your conversion rate sits below competitors' until you approach their league. This converts the wall into time — time you must finance (Sheet 8).")

note(ws, "B46", "Textiles & plant materials (v4): anything textile sold in DE needs fibre-composition labelling per EU Textile Regulation 1007/2011 (TextilKennzVO enforcement) — 'Polyester' is a defined term, marketing names are illegal on the label. Buckwheat / spelt / other plant fillings (e.g. japanisches kissen) additionally risk phytosanitary import rules + food-contact-adjacent scrutiny: confirm the filling's import status BEFORE sampling. Add both to Block A row 8/13 thinking.")
...
widths(ws, {"A": 2, "B": 40, "C": 12, "D": 11, "E": 9, "F": 11, "G": 24, "H": 2, "I": 34, "J": 12}); backlink(ws)
...
put(ws, "I5", "CPR launch push (v4)", F_SECTION, fill=FILL_HDR); put(ws, "J5", "", F_SECTION, fill=FILL_HDR)
put(ws, "I6", "CPR — units in 8 days (Cerebro)", F_BASE); inp(ws, "J6", 39, fmt="0")
comment(ws, "J6", "Helium 10's estimate of units sold via this keyword in 8 days to reach page 1. It is a heuristic, not physics — treat the total below as an order of magnitude.")
put(ws, "I7", "Launch discount per unit (EUR)", F_BASE); inp(ws, "J7", 3.0, fmt=EUR)
put(ws, "I8", "Goods burned by the push", F_BASE); frm(ws, "J8", "=IFERROR(J6*('6 Order size'!C15+J7),\"\")", fmt=EUR)
put(ws, "I9", "8 days of launch ads", F_BASE); frm(ws, "J9", '=IFERROR(C18*8,"")', fmt=EUR)
put(ws, "I10", "Total cost of the CPR push", F_BOLD); frm(ws, "J10", '=IFERROR(J8+J9,"")', fmt=EUR, font=F_BOLD)
note(ws, "I12", "Compare against Sheet 3 margin × CPR units: if the push costs more than ~6 weeks of expected page-1 contribution, the keyword is too expensive to attack head-on — launch on a cheaper child keyword first.")
...
put(ws, "B37", "Cash cycle (days per euro, v4)", F_BOLD); frm(ws, "C37", "='2 Order timing'!C18+45+14", fmt="0", font=F_BOLD)
comment(ws, "C37", "Days one euro is locked up: full supply lead time (Sheet 2 total) + ~45 days average sell-through of the batch + ~14-day Amazon payout lag. Shorten any leg and every euro works more often.")
put(ws, "B38", "Cash turns per year", F_BOLD); frm(ws, "C38", '=IFERROR(ROUND(365/C37,1),"")', fmt="0.0", font=F_BOLD)
comment(ws, "C38", "How many times a year each euro completes the loop cash → goods → sale → payout. At ~1.9 turns, a 40% per-cycle margin is ~76% a year on working capital — and why cutting lead time can beat raising price. Feeds the intuition behind Sheet 12.")
...
# =====================================================================
# 12 MULTI-YEAR PLAN (v4)
# =====================================================================
ws = wb.create_sheet("12 Multi-year plan"); ws.sheet_properties.tabColor = TAB["operate"]
w12 = {"A": 2, "B": 44}
for j in range(12): w12[get_column_letter(3 + j)] = 10
widths(ws, w12); backlink(ws)
put(ws, "B2", "Multi-year plan — 12 quarters, up to 3 staggered launches", F_TITLE)
put(ws, "B3", "The business-level question Sheet 8 cannot answer: if product 2 launches while product 1 is still ramping, does the combined plan ever run out of cash? Quarterly and deliberately coarse — Sheet 8 stays the fine-grained monthly check per product.", F_BASE, wrap=True)

put(ws, "B5", "Global inputs", F_SECTION, fill=FILL_HDR); put(ws, "C5", "", F_SECTION, fill=FILL_HDR)
put(ws, "B6", "Starting cash (from Sheet 8)", F_BASE); frm(ws, "C6", "='8 Cash flow'!C15", fmt=NUM, font=F_LINK)
put(ws, "B7", "Fixed costs per quarter (EUR)", F_BASE); inp(ws, "C7", 400, fmt=NUM)
note(ws, "E7", "Helium 10, LUCID/EPR fees, accountant, software — everything that bills regardless of sales.")
put(ws, "B8", "Launch-quarter sales share (ramp)", F_BASE); inp(ws, "C8", 0.4, fmt=PCT)
comment(ws, "C8", "A product sells this share of its mature rate in its launch quarter, halfway between this and 100% in its second quarter, and 100% from its third. 40% is a decent launch; slow niches run 25%.")

put(ws, "B10", "Products (leave a column empty to plan fewer)", F_SECTION, fill=FILL_HDR)
for col, h in [("C", "Product 1"), ("D", "Product 2"), ("E", "Product 3")]:
    put(ws, f"{col}10", h, F_SECTION, fill=FILL_HDR)
put(ws, "B11", "Launch quarter (1–12)", F_BASE)
inp(ws, "C11", 1, fmt="0"); inp(ws, "D11", 3, fmt="0"); inp(ws, "E11", None, fmt="0")
put(ws, "B12", "Mature sales, units per QUARTER", F_BASE)
inp(ws, "C12", 300, fmt=NUM); inp(ws, "D12", 300, fmt=NUM); inp(ws, "E12", None, fmt=NUM)
put(ws, "B13", "Payout per unit after tax (EUR)", F_BASE)
frm(ws, "C13", "='3 Unit economics'!C26+'3 Unit economics'!C28", fmt=EUR, font=F_LINK)
inp(ws, "D13", 20.0, fmt=EUR); inp(ws, "E13", None, fmt=EUR)
note(ws, "F13", "Product 1 pulls from Sheet 3 (Amazon payout minus 1% turnover tax). For products 2–3, run Sheet 3 per product and type the result.")
put(ws, "B14", "Landed cost per unit (EUR)", F_BASE)
frm(ws, "C14", "='6 Order size'!C15", fmt=EUR, font=F_LINK)
inp(ws, "D14", 13.0, fmt=EUR); inp(ws, "E14", None, fmt=EUR)
put(ws, "B15", "One-off launch cash (first PO + compliance + launch extras)", F_BASE)
inp(ws, "C15", 6000, fmt=NUM); inp(ws, "D15", 6500, fmt=NUM); inp(ws, "E15", None, fmt=NUM)
comment(ws, "C15", "Everything spent before the first sale: first order (Sheet 6 × landed), GPSR responsible person, LUCID/PPWR, photos, Vine units, launch-phase extra ads (Sheet 7 launch budget + CPR push). It leaves the quarter BEFORE the launch quarter (goods lead time ≈ one quarter); for a quarter-1 launch it leaves in quarter 1.")
put(ws, "B16", "Ongoing ads per unit sold (EUR)", F_BASE)
inp(ws, "C16", 3.0, fmt=EUR); inp(ws, "D16", 3.0, fmt=EUR); inp(ws, "E16", None, fmt=EUR)
note(ws, "F16", "Steady-state ad cost per unit ≈ target ACOS × gross price (Sheet 7).")

for col, y in [("C", "Year 1"), ("G", "Year 2"), ("K", "Year 3")]:
    put(ws, f"{col}18", y, F_SECTION)
put(ws, "B19", "Quarter", F_SECTION, fill=FILL_HDR)
for j in range(12):
    put(ws, f"{get_column_letter(3 + j)}19", j + 1, F_SECTION, fill=FILL_HDR, fmt='"Q"0')

prod_cols = ["C", "D", "E"]
for p in range(3):
    r = 20 + p
    pc = prod_cols[p]
    put(ws, f"B{r}", f"Units sold — product {p + 1}", F_BASE)
    for j in range(12):
        X = get_column_letter(3 + j)
        frm(ws, f"{X}{r}",
            f'=IF(OR(${pc}$11="",{X}$19<${pc}$11),0,'
            f'ROUND(${pc}$12*IF({X}$19-${pc}$11=0,$C$8,IF({X}$19-${pc}$11=1,($C$8+1)/2,1)),0))',
            fmt=NUM)

put(ws, "B24", "Cash in (payouts after tax)", F_BASE)
put(ws, "B25", "Stock purchases (next quarter's units)", F_BASE)
put(ws, "B26", "Launch one-offs", F_BASE)
put(ws, "B27", "Ongoing ads", F_BASE)
put(ws, "B28", "Fixed costs", F_BASE)
put(ws, "B29", "Net cash", F_BOLD)
put(ws, "B30", "Cumulative cash", F_BOLD)
for j in range(12):
    X = get_column_letter(3 + j)
    Y = get_column_letter(4 + j)  # next quarter's column
    frm(ws, f"{X}24", f'=ROUND({X}20*N($C$13)+{X}21*N($D$13)+{X}22*N($E$13),0)', fmt=NUM)
    if j < 11:
        frm(ws, f"{X}25",
            f'=ROUND(IF(OR($C$11="",{Y}$19=$C$11),0,{Y}20)*N($C$14)'
            f'+IF(OR($D$11="",{Y}$19=$D$11),0,{Y}21)*N($D$14)'
            f'+IF(OR($E$11="",{Y}$19=$E$11),0,{Y}22)*N($E$14),0)', fmt=NUM)
    else:
        frm(ws, f"{X}25", "=0", fmt=NUM)
    frm(ws, f"{X}26",
        f'=SUMPRODUCT(($C$11:$E$11={X}$19+1)*$C$15:$E$15)'
        f'+IF({X}$19=1,SUMPRODUCT(($C$11:$E$11=1)*$C$15:$E$15),0)', fmt=NUM)
    frm(ws, f"{X}27", f'=ROUND({X}20*N($C$16)+{X}21*N($D$16)+{X}22*N($E$16),0)', fmt=NUM)
    frm(ws, f"{X}28", "=$C$7", fmt=NUM)
    frm(ws, f"{X}29", f'={X}24-{X}25-{X}26-{X}27-{X}28', fmt=NUM, font=F_BOLD)
    if j == 0:
        frm(ws, f"{X}30", f'=$C$6+{X}29', fmt=NUM, font=F_BOLD)
    else:
        W = get_column_letter(2 + j)
        frm(ws, f"{X}30", f'={W}30+{X}29', fmt=NUM, font=F_BOLD)
